...
class mav_dynamics:
    def __init__(self, Ts):
        ...
        # wind in NED frame in meters/sec
    ...
    def _motor_thrust_torque(self, Va, delta_t):
        ...
        a = rho*np.power(D,5)*C_Q0/(2*np.pi)**2
        # b = rho*np.power(D,4)*C_Q1*self._Va/(2*np.pi) + KQ*K_V/R
        b = rho*np.power(D,4)*C_Q1*Va/(2*np.pi) + KQ**2/R
        c = rho*np.power(D,3)*C_Q2*(Va)**2 - KQ*V_in/R + KQ_i0
        ...
        T_p = rho*np.power(D,4)*omega_op**2*CT/(2*np.pi)**2
        Q_p = rho*np.power(D,5)*omega_op**2*CQ/(2*np.pi)**2
        return T_p, Q_p
